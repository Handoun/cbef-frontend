...
export default function Chat() {
  ...
  useEffect(() => {
    if (!token) return;
    axios.get(`${API_URL}/api/users`, { headers: { Authorization: `Bearer ${token}` } }).then(res => setUsers(res.data)).catch(console.error);
    axios.get(`${API_URL}/api/contacts`, { headers: { Authorization: `Bearer ${token}` } }).then(res => setContacts(res.data)).catch(console.error);
  }, [token]);
  ...
  useEffect(() => {
    const newPeer = new Peer(currentUser.id.toString(), { host: PEER_HOST, port: parseInt(PEER_PORT), path: PEER_PATH, secure: true });
    setPeer(newPeer);
  ...
  useEffect(() => {
    ...
    axios.get(`${API_URL}/api/messages/${selectedUser.id}`, { headers: { Authorization: `Bearer ${token}` } }).then(res => setMessages(res.data)).catch(console.error);
  }, [selectedUser, token]);
  ...
  const startCall = async () => {
    ...
    try {
      ...
      call.on('stream', (remoteStream: MediaStream) => {
        if (remoteAudioRef.current) { remoteAudioRef.current.srcObject = remoteStream; remoteAudioRef.current.play().catch(console.error); }
      });
  ...
  const addContact = (userId: number) => {
    axios.post(`${API_URL}/api/contacts`, { contactId: userId }, { headers: { Authorization: `Bearer ${token}` } })
      .then(() => { const newContact = users.find(u => u.id === userId); if (newContact) setContacts(prev => [...prev, newContact]); }).catch(console.error);
  };
    ...
    <div style={styles.container}>
      ...
      <div style={styles.sidebar}>
        <div style={styles.sidebarHeader}>
          <h2 style={styles.sidebarTitle}>💬 Мессенджер</h2>
        ...
        <div style={styles.contactList}>
          {displayedContacts.map(user => (
            <div key={user.id} style={{ ...styles.contactItem, backgroundColor: selectedUser?.id === user.id ? 'var(--hover-bg)' : 'transparent' }} onClick={() => setSelectedUser(user)}>
              ...
              {!contacts.some(c => c.id === user.id) && (
                <button style={styles.addButton} onClick={(e) => { e.stopPropagation(); addContact(user.id); }}>+ Добавить</button>
              )}
            </div>
          ))}
        </div>
      ...
      {selectedUser ? (
        <div style={styles.chatArea}>
          <div style={styles.chatHeader}>
            <h3 style={styles.chatHeaderTitle}>{selectedUser.username} <span style={{ marginLeft: 8, fontSize: 14, fontWeight: 400 }}>{onlineUsers.has(selectedUser.id) ? '🟢 онлайн' : '⚫ офлайн'}</span></h3>
            <div>{!currentCall ? <button style={styles.callButton} onClick={startCall}>📞 Позвонить</button> : <button style={styles.hangUpButton} onClick={hangUp}>❌ Завершить</button>}</div>
          </div>
          <div style={styles.messagesContainer}>
            {messages.map(msg => (
              <div key={msg.id} style={{ ...styles.messageRow, justifyContent: msg.sender_id === currentUser.id ? 'flex-end' : 'flex-start' }}>
                <div style={{ ...styles.messageBubble, ...(msg.sender_id === currentUser.id ? styles.messageOwn : styles.messageOther) }}>
                  {msg.image && <img src={msg.image} alt="sent" style={{ maxWidth: '100%', borderRadius: 8, marginBottom: 4 }} />}
          ...
          <div style={styles.inputContainer}>
            ...
            <input style={styles.input} value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendMessage()} placeholder="Введите сообщение" />
            <button style={styles.sendButton} onClick={() => sendMessage()}>Отправить</button>
          </div>
          {showEmoji && <div style={{ position: 'absolute', bottom: '80px', right: '20px' }}><EmojiPicker onEmojiClick={(e) => setInput(prev => prev + e.emoji)} /></div>}
          {showStickers && (
            <div style={{ ...styles.attachmentMenu, bottom: '130px' }}>
              <div style={styles.stickerGrid}>{stickers.map(s => <div key={s} style={styles.sticker} onClick={() => sendSticker(s)}>{s}</div>)}</div>
            </div>
          )}
        </div>
      ) : (
        <div style={styles.emptyChat}>Выберите контакт, чтобы начать общение</div>
      )}
